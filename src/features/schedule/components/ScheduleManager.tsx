...
import type { Aluno, Psicologo } from "../../../features/auth/contexts/UserDataProvider";

// Define os tipos de eventos que o calendário pode exibir
export type CalendarEvent = {
  id: string;
  start: string;
  title: string;
  status: Consulta['status'];
  participant?: Aluno | Psicologo;
};
...
export default function ScheduleManager({
  ...
  const handleDayClick = (day: Date) => {
    ...
    const hasEvents = consultas.some(event => formatDateKey(new Date(event.horario)) === dayKey);
    ...
    <div className="relative h-full w-full flex flex-row overflow-hidden rounded-2xl">
      ...
      {selectedDayForDetail && (
        <DayDetailPanel
          ...
          eventsForDay={consultas.filter(c => formatDateKey(new Date(c.horario)) === formatDateKey(selectedDayForDetail))}
